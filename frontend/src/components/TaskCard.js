...
export default function TaskCard({ task, showAssignees = true }) {
  ...
  return (
    <button
      ...
      className="group relative w-full h-full flex flex-col text-left rounded-2xl p-4 pl-5 border shadow-card hover:shadow-card-hover hover:-translate-y-0.5 transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-[#D4AF37]"
      style={{ backgroundColor: colors.bg.card, borderColor: colors.border.subtle }}
    >
      <div className="absolute top-0 bottom-0 left-0 w-1 rounded-l-2xl" style={{ backgroundColor: colors.priority[task.priority] }} />
      <div className="flex justify-between items-start gap-2 mb-2">
        <div className="flex-1 pr-2 min-w-0">
          {task.project_name ? (
            <p className="text-[10px] tracking-[1.5px] font-bold uppercase mb-1 truncate" style={{ color: colors.brand.gold }}>{task.project_name}</p>
          ) : task.category ? (
            <p className="text-[10px] tracking-[1.5px] font-bold uppercase mb-1 truncate" style={{ color: colors.brand.maroon }}>{task.category}</p>
          ) : null}
          <h3 className="text-[15px] font-bold leading-snug line-clamp-2" style={{ color: colors.text.primary }}>{task.title}</h3>
        </div>
      ...
      {showAssignees && task.assignments.length > 0 && (
        <div className="flex items-center gap-2 mt-3 pt-3 border-t" style={{ borderColor: colors.border.subtle }}>
